    ...
    for gb_record in SeqIO.parse(open(gb_file,"r"),"genbank"):
        ...
        for feature in fs:
            ...
            if feature.type=="CDS":
                ...
                name = str(feature.qualifiers['product'])
                ...
                start = start + 1
                local_feature_stat=(plasmid,name,start,end,str(seq),note)
            else:
                continue
            feature_data_bank.append(local_feature_stat)

#Aca printeamos los datos en un texto plano#            
with open("multifastp_d.fa","w") as multifastap:
    protein_fasta_list = []
    for index,value in enumerate(feature_data_bank):
        if "copper" in value[1] or "Copper" in value[1] or "Arsenic" in value[1] or "arsenic" in value[1] or "mercur" in value[1] or "Mercur" in value[1] or "Tellurium" in value[1] or "tellurium" in value[1]:
            protein_fasta = ""
            header = ">"
            resistance = "_METAL_RESISTANCE"
            plasmid = value[0]
            connector = "_"
            start = value[2]
            end = value[3]
            jump  = "\n"
            seq = value[4]
            protein_fasta = header + plasmid + connector + str(start) + connector + str(end) + resistance + jump + seq + jump
            protein_fasta_list.append(protein_fasta)
            multifastap.write(protein_fasta)  
            continue
        if "pump" in value[5] or "pump" in value[1]:
            protein_fasta = ""
            header = ">"
            pump = "_PUMP"
            plasmid = value[0]
            ...
            jump  = "\n"
            seq = value[4]
            protein_fasta = header + plasmid + connector + str(start) + connector + str(end) + pump + jump + seq + jump
            protein_fasta_list.append(protein_fasta)
            multifastap.write(protein_fasta)  
            continue
        if "penem" in value[5] or "lactamase" in value[5] or "carbapenem" in value[5] or "penem" in value[1] or "lactamase" in value[1] or "carbapenem" in value[1]:
            protein_fasta = ""
            header = ">"
            pene = "_CARBAPENEM"
            plasmid = value[0]
            ...
            jump  = "\n"
            seq = value[4]
            protein_fasta = header + plasmid + connector + str(start) + connector + str(end) + pene + jump + seq + jump
            protein_fasta_list.append(protein_fasta)
            multifastap.write(protein_fasta)  
            continue
        if "Antibiotic" in value[5] or "antibiotic" in value[5] or "Antibiotic" in value[1] or "antibiotic" in value[1] or "Tetracycline" in value[1] or "tetracycline" in value[1]:
            protein_fasta = ""
            header = ">"
            ant = "_ANTIBIOTIC"
            plasmid = value[0]
            ...
            jump  = "\n"
            seq = value[4]
            protein_fasta = header + plasmid + connector + str(start) + connector + str(end) + ant + jump + seq + jump
            protein_fasta_list.append(protein_fasta)
            multifastap.write(protein_fasta)  
            continue

with open("multifastp_d.fa","w") as ahorasi:
